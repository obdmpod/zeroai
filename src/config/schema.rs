...
pub struct HeartbeatConfig {
    ...
    /// Optional provider for heartbeat tasks (e.g. "gemini"). When set, heartbeat uses this
    /// instead of default_provider so you can use a small/cheap model for periodic tasks.
    #[serde(default)]
    pub provider: Option<String>,
    /// Optional model for heartbeat tasks (e.g. "gemini-2.0-flash-lite"). When set, heartbeat
    /// uses this instead of default_model. Ignored if provider is not set.
    #[serde(default)]
