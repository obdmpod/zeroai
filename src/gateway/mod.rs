...
/// Embedded web UI (pair + chat) — single HTML file
...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    ...
    // Build router: API routes first, then fallback to serve web UI for GET
    let app = Router::new()
...
/// Fallback: serve web UI (pair + chat) for GET, 404 otherwise
async fn serve_ui(request: Request<Body>) -> impl IntoResponse {
