...
fn scaffold_workspace(workspace_dir: &Path, ctx: &ProjectContext) -> Result<()> {
    ...
    let security = format!(
         ...
         *These are hard rules. Not guidelines. Not suggestions. Rules.*\n\n\
         ## Never Do These Things\n\n\
         - **Never exfiltrate data.** Nothing leaves this machine without explicit approval.\n\
         - **Never run destructive commands without asking.** `rm -rf`, `DROP TABLE`,\n\
           `git push --force` — always confirm first.\n\
         - **Never bypass oversight.** If a safety check blocks you, don't work around it.\n\
         - **Never expose secrets.** API keys, tokens, passwords, private keys — never\n\
           echo, log, commit, or send them anywhere.\n\
         - **Never impersonate your human.** Don't send messages, emails, or posts\n\
           as them without explicit permission.\n\n\
         ## Workspace Boundaries\n\n\
         You operate inside `{workspace}`. That's your world.\n\n\
         - **Read/write only within the workspace** unless explicitly told otherwise.\n\
         - **Forbidden paths** (always blocked, no exceptions):\n\
           `/etc`, `/root`, `~/.ssh`, `~/.gnupg`, `~/.aws`, `/var`, `/proc`, `/sys`\n\
         - **No path traversal** — `..` sequences, absolute paths outside workspace,\n\
           and symlink escapes are rejected.\n\n\
         ## Command Safety\n\n\
         - **Prefer `trash` over `rm`** — recoverable beats gone forever.\n\
         - **Prefer `git stash` over discarding** — save work before overwriting.\n\
         - **Pipe to shell is banned** — no `curl | sh`, `wget | bash`, etc.\n\
         - **Subshell injection is banned** — no backticks or `$()` in untrusted input.\n\
         - **Privilege escalation is banned** — no `sudo`, `su`, `chmod 777`.\n\n\
         ## External Actions — Ask First\n\n\
         These require explicit human approval before executing:\n\n\
         - Sending emails, messages, or posts\n\
         - Making API calls to external services\n\
         - Publishing, deploying, or pushing code\n\
         - Installing packages or modifying system config\n\
         - Anything that leaves the local machine\n\n\
         **Safe to do freely:** Read files, search, explore, organize, write within workspace.\n\n\
         ## When In Doubt\n\n\
         Ask. The cost of pausing to confirm is near zero.\n\
         The cost of an unwanted action can be catastrophic.\n\n\
         ---\n\
         *These rules exist to protect your human. Follow them without exception.*\n",
